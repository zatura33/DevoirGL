...
public class ControllerPaiement implements Initializable
{
	...
	void OnClickPayer(ActionEvent event) 
	{
		...
		// Le paiement est effectu� nous pouvons donc cr�er le paiement, le stocker dans la BDD et mettre la facture en mode estPaye en true
		Paiement paiement = facture.CreatePaiement(this.methodePaiement);
		facture.setEstPaye(true);
		DataBase.AddPaiement(paiement);
		new Alert(Alert.AlertType.CONFIRMATION, "Paiement effectu� avec succes.").showAndWait();
		EffectuerPaiement();
		try 
		{
